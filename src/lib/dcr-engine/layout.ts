import { EventMap, RelationType, Event, DCRGraph, DCRGraphS, Nestings } from "./types";
...
const createXML = (laidOutGraph: LayoutType, nodesAndEdges: AbstractGraph, graph: DCRGraphS, nestings?: Nestings) => {
    ...
    const createNodeArrayXML = (nodes: Array<AbstractNode>, nestings: Nestings): string => {
        ...
        nodes.forEach((node) => {
            if (nestings.nestingIds.has(node.id)) {
                retval += `<dcr:nesting id="${descToId(node.id)}" description="${graph.labelMap[node.id] || node.id}">\n`;
                if (node.children) retval += createNodeArrayXML(node.children, nestings);
                retval += "</dcr:nesting>\n";
            } else {
                retval += ` <dcr:event id="${descToId(node.id)}" description="${graph.labelMap[node.id] || node.id}" included="${node.included}" executed="${node.executed}" pending="${node.pending}" enabled="false" />\n`;
            }
        })
        return retval;
    }
    ...
    if (nestings) {
        ...
        nodesAndEdges.nodes.forEach((node) => {
            xmlContent += ` <dcr:event id="${descToId(node.id)}" description="${graph.labelMap[node.id] || node.id}" included="${node.included}" executed="${node.executed}" pending="${node.pending}" enabled="false" />\n`;
        })
    }
...
const getAbstractGraph = (graph: DCRGraphS, nestings?: Nestings): AbstractGraph => {
...
const layoutGraph = async (graph: DCRGraphS, nestings?: Nestings) => {
    ...
    const xmlContent = createXML(result, abstractGraph, graph, nestings);
