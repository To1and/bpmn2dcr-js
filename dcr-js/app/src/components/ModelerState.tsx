...
import { DCRGraph, DCRGraphS, layoutGraph, moddleToDCR, nestDCR, Nestings } from 'dcr-engine';
  ...
  const layout = () => {
    ...
    if (confirm("This will overwrite your current layout, do you wish to continue?")) {
      try {
        ...
        const params: [DCRGraphS, Nestings | undefined] = nest ? [nestings.nestedGraph, nestings] : [graph, undefined];
        layoutGraph(...params).then(xml => {
          modelerRef.current?.importDCRPortalXML(xml).catch(e => {
            console.log(e);
