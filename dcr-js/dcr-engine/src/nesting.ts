import { DCRGraph, DCRGraphS, Event, EventMap, Marking, RelationType } from "./types";
...
const relationsDictToDCR = (relationsDict: RelationsDict, originalGraph: DCRGraphS, marking?: Marking): DCRGraphS => {
    const events = new Set(Object.keys(relationsDict));
    const graph: DCRGraphS = {
        events,
        ...
        // Copy label and role information from original graph
        labels: new Set(originalGraph.labels),
        labelMap: { ...originalGraph.labelMap },
        labelMapInv: { ...originalGraph.labelMapInv },
        roles: new Set(originalGraph.roles),
        roleMap: { ...originalGraph.roleMap },
        subProcesses: { ...originalGraph.subProcesses },
        subProcessMap: { ...originalGraph.subProcessMap }
    }
...
export const nestDCR = (graph: DCRGraphS, minSharedRels: number = 1, events = copySet(graph.events), relationDict: RelationsDict = getRelationsDict(graph)): ReturnNesting<DCRGraphS> => {
    ...
    return { nestedGraph: { ...relationsDictToDCR(relationDict, graph, graph.marking), events: copySet(graph.events).union(nestingIds) }, nestingIds, nestingRelations };
}